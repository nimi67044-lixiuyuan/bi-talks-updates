...
electron.contextBridge.exposeInMainWorld("__biTalksReportUnread", (sample) => {
  if (sample && Number.isFinite(sample.count)) electron.ipcRenderer.send("service:unread", { ...sample, accountId });
});
...
if (platform === "whatsapp") {
  electron.contextBridge.executeInMainWorld({
    func: () => {
      ...
      if (state.__biTalksCachePutSerialized || typeof Cache === "undefined") return;
      state.__biTalksCachePutSerialized = true;
      ...
      Cache.prototype.put = function put(request, response) {
        ...
        const key = typeof request === "string" ? new URL(request, location.href).href : request instanceof URL ? request.href : request.url;
        const previous = queue.get(key) || Promise.resolve();
        const operation = previous.catch(() => void 0).then(async () => {
          try {
            await originalPut.call(this, request, response);
          } catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            if (!/Entry already exists/i.test(message)) throw error;
          }
        });
...
const retryAfter = /* @__PURE__ */ new WeakMap();
...
function appendTranslation(host, text, translated, key) {
  ...
  if (existing?.dataset.sourceKey === key) {
...
async function translateHost(host) {
  if ((retryAfter.get(host) || 0) > Date.now()) return;
  ...
  try {
    ...
    appendTranslation(host, text, "正在翻译…", key);
    const requestId = crypto.randomUUID();
    let partial = "";
    const current = () => host.isConnected && visibleHosts.has(host) && chatIsReady() && messageText(host) === text;
    const listener = (_event, payload) => {
      if (payload.requestId !== requestId || !current()) return;
      partial += payload.text;
      appendTranslation(host, text, partial, key);
    };
    electron.ipcRenderer.on("translate:stream:delta", listener);
    const cancellation = setInterval(() => {
      if (!current()) void electron.ipcRenderer.invoke("translate:cancel", requestId).catch(() => void 0);
    }, 250);
    let translated;
    try {
      translated = await electron.ipcRenderer.invoke("translate:stream", requestId, text, "zh", "en", !!host.closest(".message-out"));
    } catch (error) {
      existingTranslation(host)?.remove();
      throw error;
    } finally {
      clearInterval(cancellation);
      electron.ipcRenderer.removeListener("translate:stream:delta", listener);
    }
    if (current()) appendTranslation(host, text, translated, key);
  } catch {
    processed.delete(host);
    retryAfter.set(host, Date.now() + 3e4);
  } finally {
...
function scan() {
  ...
  scanRunning = true;
  try {
    for (const host of [...pendingHosts].sort((a, b) => b.getBoundingClientRect().bottom - a.getBoundingClientRect().bottom)) {
