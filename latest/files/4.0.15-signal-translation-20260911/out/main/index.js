...
class Translator {
  ...
  async translate(text, target, source = "auto", _onDelta, options = {}) {
    ...
    const key = createHash("sha256").update(JSON.stringify([identity, cacheSource, targetLanguage, text, !!options.bypassCache, options.timeoutMs || 18e4])).digest("hex");
    const existing = this.pending.get(key);
    if (existing) return await existing;
    const task = (async () => {
      if (options.isCurrent && !await options.isCurrent()) throw new Error("翻译任务已取消。");
      ...
      return this.scheduler.run(async () => {
        ...
        const rawResult = await translateWithDeepL(text, targetLanguage, sourceLanguage, options.timeoutMs);
...
function installSignalTranslation() {
  ...
  if (signalWindow.__biTalksSignalTranslation?.version === 6) return;
...
async function createWindow() {
  ...
  signal = new SignalManager(
    emit,
    (text, accountId, onDelta, isCurrent) => translator.translate(text, "zh", "en", onDelta, { scope: accountId, isCurrent, timeoutMs: 3e4 }),
...
authenticatedHandle("translate:stream", (event, requestId, text, target, source) => {
  ...
  }, { scope: translationScope(event), isCurrent: () => !task.cancelled && !event.sender.isDestroyed() }).finally(() => translationRequests.delete(key));
});
