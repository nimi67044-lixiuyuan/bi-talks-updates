...
class SignalTranslationBridge {
  ...
  async connect(session2, initial) {
    try {
      ...
      const unreadScript = await readFile(join(app.isPackaged && !process.env.ELECTRON_RENDERER_URL && existsSync(join(process.resourcesPath, "vendor/webview2-host/UnreadMonitor.js")) ? process.resourcesPath : app.getAppPath(), "vendor/webview2-host/UnreadMonitor.js"), "utf8");
      const unreadSource = unreadScript + '("signal", sample => window.__biTalksUnreadReport(JSON.stringify(sample)))';
      await connection.call("Page.addScriptToEvaluateOnNewDocument", { source: unreadSource });
      await connection.call("Runtime.evaluate", { expression: unreadSource });
