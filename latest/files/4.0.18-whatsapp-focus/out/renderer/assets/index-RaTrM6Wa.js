...
function isAccountHidden(account) {
  return (account.platform === "telegram" || account.platform === "whatsapp") && account.hidden === true;
}
const platforms = {
  ...
  googlechat: { name: "Google Chat", color: "#34a853" },
...
function PlatformIcon({ platform, className = "platform-icon" }) {
  ...
  if (platform === "zoom") {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `${className} platform-icon-fallback zoom-icon`, "aria-hidden": "true", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Video, { size: 19 }) });
  }
  if (platform === "googlechat") {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: className + " platform-icon-fallback", style: { color: "#34a853" }, "aria-hidden": "true", children: /* @__PURE__ */ jsxRuntimeExports.jsx(MessagesSquare, { size: 21 }) });
  }
...
function UpdateCheckButton({
  ...
  const patchSize = availablePatch ? availablePatch.downloadSize < 1024 * 1024 ? `${Math.max(1, Math.ceil(availablePatch.downloadSize / 1024))} 千字节` : `${(availablePatch.downloadSize / (1024 * 1024)).toFixed(1)} 兆字节` : "";
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    buttonHost && reactDomExports.createPortal(
      /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
        {
          className: "ghost-button settings-update-check-button",
          type: "button",
          title: "检查更新",
          "aria-label": "检查更新",
          disabled: checking,
...
const webPlatforms = /* @__PURE__ */ new Set(["whatsapp", "telegram", "googlevoice", "zoom", "googlechat"]);
const signalDesktopSupported = window.desktop.platform === "win32";
const translationPreferencePlatforms = /* @__PURE__ */ new Set(["signal", "whatsapp", "googlechat"]);
const translationTogglePlatforms = /* @__PURE__ */ new Set(["signal", "whatsapp", "googlechat"]);
const themeOptions = [
...
function Modal({ title, children, onClose }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "modal-backdrop", onMouseDown: onClose, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal", onMouseDown: (event) => event.stopPropagation(), children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-title", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "icon-button", onClick: onClose, "aria-label": "关闭", children: /* @__PURE__ */ jsxRuntimeExports.jsx(X, { size: 18 }) })
    ] }),
    children
  ] }) });
}
function conversationTitle(id) {
  if (!id || id === "default") return "新会话";
  return id;
}
...
function accountCardStatus(value) {
  ...
  return /^(?:ready|.+(?:\swindow)?\sready)$/i.test(status) ? void 0 : status.replace(/^Managed Signal closed$/i, "Signal 已关闭");
}
function setAccountUnread(prev, accountId, count) {
  const nextCount = Math.max(0, Math.min(999, Math.round(count)));
  if (nextCount <= 0) {
...
function applyExternalUnread(prev, accountId, count) {
  const nextCount = Math.max(0, Math.min(999, Math.round(count)));
...
function summarizeConversations(messages, accountId, draftId = "") {
  ...
  for (const message of messages) {
    ...
    byId.set(message.conversationId, {
      ...
      lastText: message.translatedText || message.originalText || "附件",
...
function App() {
  return /* @__PURE__ */ jsxRuntimeExports.jsx(AuthenticatedApp, {});
}
function AuthenticatedApp() {
  ...
  const [pendingAppPlatform, setPendingAppPlatform] = reactExports.useState();
  const appAccountAdded = reactExports.useRef(false);
  const [utilityWorkspace, setUtilityWorkspace] = reactExports.useState();
  const settingsOpen = utilityWorkspace === "settings";
  const [updateDialogOpen, setUpdateDialogOpen] = reactExports.useState(false);
  const [updateButtonHost, setUpdateButtonHost] = reactExports.useState(null);
  ...
  reactExports.useEffect(() => {
    const visibleAccountIds = new Set((state?.accounts || []).filter((account) => !isAccountHidden(account)).map((account) => account.id));
    const total = Object.entries(unreadCounts).reduce(
      (sum, [accountId, count]) => sum + (visibleAccountIds.has(accountId) ? count : 0),
      0
    );
  ...
  reactExports.useEffect(() => {
    ...
    const unsubscribe = window.desktop.onEvent((event) => {
      ...
      if (event.type === "error") {
        setNotice(event.message);
        if (event.accountId) setStatuses((items) => ({ ...items, [event.accountId]: "连接失败" }));
      }
    });
    void window.desktop.getState().then((next) => {
      if (!disposed) {
        setState(next);
        if (!next.accounts.some((account) => account.platform === next.selectedPlatform)) setUtilityWorkspace("apps");
      }
    });
  ...
  const accounts = reactExports.useMemo(() => {
    ...
    const byId = new Map(state.accounts.filter((item) => item.platform === state.selectedPlatform && !isAccountHidden(item)).map((item) => [item.id, item]));
  ...
  const modalIsOpen = addOpen || !!utilityWorkspace || updateDialogOpen || !!confirmRemove || !!accountSettingsAccountId || confirmCloseOpen;
  ...
  reactExports.useEffect(() => () => window.desktop.setWorkspaceNotice(void 0), []);
  reactExports.useEffect(() => {
  ...
  reactExports.useEffect(() => {
    ...
    setNotice(void 0);
    setStatuses((items) => ({ ...items, [selectedAccount.id]: "正在启动 Signal" }));
    void window.desktop.attachSignalWindow(account, bounds).then(() => {
      ...
      setStatuses((items) => ({ ...items, [account.id]: "Signal 启动失败" }));
  ...
  const selectPlatform = async (platform) => {
    setUtilityWorkspace(void 0);
    if (!state) return;
    signalAttachRunId.current += 1;
    const rememberedAccountId = state.selectedAccountIdsByPlatform[platform];
    const visibleOnPlatform = (item) => item.platform === platform && !isAccountHidden(item);
  ...
  const startAccountAction = async (account) => {
    ...
    if (account.platform === "signal" && signalDesktopSupported) {
      ...
      if (bounds) {
        ...
        setStatuses((items) => ({ ...items, [account.id]: "正在启动 Signal" }));
  ...
  const closeAccountAction = async (account) => {
    ...
    await window.desktop.closeAccount(account).then(() => setStatuses((items) => ({ ...items, [account.id]: "已关闭" }))).catch((error) => {
      manuallyClosedAccountIds.current.delete(account.id);
      setAccountLifecycleRevision((value) => value + 1);
      setStatuses((items) => ({ ...items, [account.id]: "关闭失败" }));
      setNotice(error instanceof Error ? error.message : String(error));
    });
  };
  const updateSignalDesktopAction = async (account) => {
    ...
    setNoticeProgress(void 0);
    try {
      await window.desktop.updateSignalDesktop(account);
    } catch (error) {
      setNotice(error instanceof Error ? error.message : String(error));
    } finally {
      setAccountLifecycleRevision((value) => value + 1);
  ...
  const removeAccount = async (account) => {
    ...
    const firstVisibleAccountId = (platform, accountIds) => accountIds.find((id) => {
      ...
      return candidate?.platform === platform && !isAccountHidden(candidate);
    });
    ...
    if (!accountsLeft.some((item) => item.platform === account.platform) && state.selectedPlatform === account.platform && !utilityWorkspace) {
      setUtilityWorkspace("apps");
    }
  };
  ...
  const platformsWithAccounts = new Set(state.accounts.filter((account) => !isAccountHidden(account)).map((account) => account.platform));
  const visiblePlatformOrder = state.platformOrder.filter((platform) => platformsWithAccounts.has(platform) || pendingAppPlatform === platform);
  const availableAppPlatforms = state.platformOrder.filter((platform) => !platformsWithAccounts.has(platform));
  const openAppPlatform = async (platform) => {
    if (pendingAppPlatform) return;
    appAccountAdded.current = false;
    setPendingAppPlatform(platform);
    setNotice(void 0);
    try {
      await selectPlatform(platform);
      setAddOpen(true);
    } catch (error) {
      setPendingAppPlatform(void 0);
      setUtilityWorkspace("apps");
      setNotice(error instanceof Error ? error.message : String(error));
    }
  };
  const closeAddAccount = () => {
    setAddOpen(false);
    if (pendingAppPlatform && !appAccountAdded.current) setUtilityWorkspace("apps");
    setPendingAppPlatform(void 0);
    appAccountAdded.current = false;
  };
  ...
  for (const account of state.accounts) {
    if (isAccountHidden(account)) continue;
  ...
  const handleAccountAdded = (account) => {
    if (pendingAppPlatform === account.platform) appAccountAdded.current = true;
    setStatuses((items) => ({ ...items, [account.id]: `正在启动 ${platforms[account.platform].name}…` }));
  };
      ...
      "div",
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("aside", { className: "rail-panel", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "platform-rail", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: `platform-button rail-apps-button ${utilityWorkspace === "apps" ? "utility-active" : ""}`, onClick: () => {
              hideSignalForOverlayUi();
              setNotice(void 0);
              setUtilityWorkspace("apps");
            }, type: "button", title: "应用", "aria-label": "应用", children: /* @__PURE__ */ jsxRuntimeExports.jsx(LayoutGrid, { size: 20 }) }),
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(DndContext, { sensors, collisionDetection: closestCenter, onDragEnd: platformDrop, children: /* @__PURE__ */ jsxRuntimeExports.jsx(SortableContext, { items: visiblePlatformOrder, strategy: verticalListSortingStrategy, children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "platform-list", children: visiblePlatformOrder.map((platform) => {
              return /* @__PURE__ */ jsxRuntimeExports.jsx(SortableItem, { id: platform, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "button",
                {
                  className: `platform-button ${!utilityWorkspace && state.selectedPlatform === platform ? "active" : ""}`,
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "rail-bottom", style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }, children: /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: `platform-button ${settingsOpen ? "utility-active" : ""}`, title: "设置", onClick: openSettings, children: /* @__PURE__ */ jsxRuntimeExports.jsx(Settings, { size: 20 }) }) })
          ] }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("aside", { ref: accountPanel, className: "account-panel-shell", style: utilityWorkspace ? { visibility: "hidden" } : void 0, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "account-panel", children: [
                ...
                "div",
                {
                  ...
                  children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "account-avatar", children: [
                      ...
                      !!unreadCounts[account.id] && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "unread-badge account-unread-badge", children: unreadLabel(unreadCounts[account.id]) })
                    ] }),
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `account-copy${cardStatus ? " has-status" : ""}`, children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsxs("strong", { children: [
                        cardStatus?.startsWith("正在") && /* @__PURE__ */ jsxRuntimeExports.jsx(LoaderCircle, { size: 12, className: "spin zoom-start-spinner" }),
                        account.label
                      ] }),
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "account-add-slot", children: /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "platform-button account-add-button", onClick: openAddAccount, title: "添加账号", "aria-label": "添加账号", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Plus, { size: 16 }) }) })
          ] }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("main", { className: `workspace-panel ${utilityWorkspace ? "utility-workspace-open" : ""}`, children: [
            utilityWorkspace && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "utility-workspace", children: [
              settingsOpen && /* @__PURE__ */ jsxRuntimeExports.jsx(
                SettingsModal,
                {
                  state,
                  save,
                  onClose: () => setUtilityWorkspace(void 0),
                  onNotice: setNotice,
                  updateButtonHostRef: setUpdateButtonHost
                }
              ),
              !settingsOpen && /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "应用" }),
              utilityWorkspace === "apps" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "available-apps", children: [
                availableAppPlatforms.map((platform) => /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "available-app-card", disabled: !!pendingAppPlatform, onClick: () => void openAppPlatform(platform), children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx(PlatformIcon, { platform }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: platforms[platform].name }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "添加账号" })
                ] }, platform)),
                !availableAppPlatforms.length && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: "所有应用均已添加账号，可从左侧切换使用。" })
              ] })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("section", { className: "workspace", style: workspaceTranslationStyle, children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { ref: contentArea, className: "content-area", children: workspaceIsWeb ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { ref: webSurface, className: "web-surface", children: workspacePreview?.accountId === selectedAccount?.id ? /* @__PURE__ */ jsxRuntimeExports.jsx("img", { className: "workspace-taskbar-preview", src: workspacePreview.dataUrl, alt: "", "aria-hidden": "true", draggable: false }) : /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "web-loading", children: [
              ...
              !selectedSignalLooksReady && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "signal-desktop-card", children: [
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { children: signalDesktopSupported ? "正在启动 Signal" : "Signal 集成组件不可用" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: signalDesktopSupported ? "此账号使用由程序管理的独立 Signal 桌面实例和数据目录，不影响桌面上已有的 Signal 窗口。" : "Signal 托管窗口需要安装包内附带的 Windows 集成组件。" }),
            ...
            ] }) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "chat-workspace no-account", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "message-list", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "conversation-empty", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(MessagesSquare, { size: 42 }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { children: "多平台翻译聊天" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: "选择一个账号，开始接收和翻译消息。" })
            ] }) }) }) }) })
          ] })
        ]
      }
    ),
    addOpen && /* @__PURE__ */ jsxRuntimeExports.jsx(AddAccount, { state, save, onClose: closeAddAccount, onAdded: handleAccountAdded }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      UpdateCheckButton,
      {
        buttonHost: updateButtonHost,
        onNotice: setNotice,
...
function AddAccount({ state, save, onClose, onAdded }) {
  ...
  const accountNameKey = (value) => value.trim().normalize("NFKC").toLocaleLowerCase();
  const submit = async () => {
    ...
    try {
      ...
      if ((platform === "telegram" || platform === "whatsapp") && label.trim()) {
        const existing = state.accounts.find((item) => item.platform === platform && accountNameKey(item.label) === accountNameKey(label));
        if (existing) {
          ...
          const nextVisibleAccountId = state.accountOrder[platform].find((id) => {
            const candidate = accounts.find((item) => item.id === id);
            return candidate?.platform === platform && !isAccountHidden(candidate);
          });
          const hidesSelectedAccount = hidden && state.selectedPlatform === platform && state.selectedAccountId === existing.id;
          const selectedAccountId = hidden ? hidesSelectedAccount ? nextVisibleAccountId : state.selectedAccountId : existing.id;
          const selectedAccountIdsByPlatform = { ...state.selectedAccountIdsByPlatform };
          if (selectedAccountId) selectedAccountIdsByPlatform[platform] = selectedAccountId;
          else delete selectedAccountIdsByPlatform[platform];
          await save({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(Modal, { title: `添加 ${platformName} 账号`, onClose, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { children: [
          "显示名称",
          /* @__PURE__ */ jsxRuntimeExports.jsx("input", { value: label, onChange: (event) => setLabel(event.target.value), placeholder: `${platformName} 账号` })
        ] }),
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-actions", children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "ghost-button", type: "button", disabled: submitting, onClick: onClose, children: "取消" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "primary-button", type: "button", disabled: submitting, onClick: () => void submit(), children: submitting ? "正在保存…" : "添加" })
        ] })
      ]
    }
  ) });
...
function ConfirmRemoveAccount({ account, onCancel, onConfirm }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(Modal, { title: "删除账号", onClose: onCancel, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "form-stack", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "confirm-copy", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("strong", { children: [
        "确定删除 ",
        account.label,
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: "此操作将从 Bi-Talks 中删除该账号，且无法撤销。" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-actions", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "ghost-button", type: "button", disabled: removing, onClick: onCancel, children: "取消" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "danger-button", type: "button", disabled: removing, onClick: () => void confirm(), children: removing ? "正在删除…" : "删除" })
    ] })
  ] }) });
}
...
function KeyConfigurationStatus({ saved, message }) {
  const isStatus = !message || message === "已配置" || message === "尚未配置 API Key。";
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { role: "status", className: isStatus ? "key-configuration-status" : void 0, children: [
    isStatus && /* @__PURE__ */ jsxRuntimeExports.jsx("i", { className: saved ? "configured" : "unconfigured" }),
    message || (saved ? "已配置" : "尚未配置 API Key。")
  ] });
}
function DeepLKeySettings() {
  const [key, setKey] = reactExports.useState("");
  const [status, setStatus] = reactExports.useState({ saved: false });
  const [busy, setBusy] = reactExports.useState(true);
  const [message, setMessage] = reactExports.useState("");
  reactExports.useEffect(() => {
    let active = true;
    window.desktop.deepLKeyStatus().then((value) => {
      if (active) setStatus(value);
    }).catch((error) => {
      if (active) setMessage(error instanceof Error ? error.message : "无法读取 DeepL API Key 状态。");
    }).finally(() => {
  ...
  const save = async () => {
    if (!key.trim()) return;
    setBusy(true);
    setMessage("正在验证 DeepL API Key…");
    try {
      const next = await window.desktop.saveDeepLKey(key);
      setStatus(next);
      setKey("");
      setMessage("已配置");
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "DeepL API Key 保存失败。");
    } finally {
      setBusy(false);
    }
  };
  const clear = async () => {
    setBusy(true);
    try {
      setStatus(await window.desktop.clearDeepLKey());
      setKey("");
      setMessage("尚未配置 API Key。");
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "DeepL API Key 清除失败。");
    } finally {
      setBusy(false);
    }
  };
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "settings-section", "aria-label": "DeepL API Key", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: "DeepL API Key" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { children: [
        "兼容 DeepL API Free 与 Pro，",
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "translation-only-note", children: "仅供翻译服务。" }),
        "Key 将安全加密。"
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "form-field", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "API Key" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("input", { type: "password", autoComplete: "off", spellCheck: false, value: key, disabled: busy, placeholder: status.saved ? status.masked || "已配置" : "输入 DeepL API Key", onChange: (event) => setKey(event.target.value) })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "inline-actions", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "primary-button key-action-button", type: "button", disabled: busy || !key.trim(), onClick: () => void save(), children: busy ? "测试中…" : "测试Key" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "ghost-button key-action-button", type: "button", disabled: busy || !status.saved, onClick: () => void clear(), children: "清除Key" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(KeyConfigurationStatus, { saved: status.saved, message })
  ] });
}
function SettingsModal({ state, save, onClose, onNotice, updateButtonHostRef }) {
  ...
  const submit = () => {
    ...
    onClose();
    void request.then(() => onNotice("设置已保存。")).catch((error) => onNotice(error instanceof Error ? error.message : "设置保存失败。"));
  };
  const updateThemeMode = (mode) => {
    setThemeMode(mode);
    void persistSettings({ themeMode: mode });
  };
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "settings-workspace", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "设置" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "form-stack", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "settings-section", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: "外观" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: "选择程序的显示主题。" })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "segmented-control", role: "radiogroup", "aria-label": "主题模式", children: themeOptions.map((option) => /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            type: "button",
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(DeepLKeySettings, {}),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-actions settings-modal-actions", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { ref: updateButtonHostRef, className: "settings-update-button-host" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "settings-modal-save-actions", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "ghost-button", onClick: onClose, children: "取消" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "primary-button", onClick: submit, children: "保存" })
        ] })
      ] })
    ] })
  ] });
