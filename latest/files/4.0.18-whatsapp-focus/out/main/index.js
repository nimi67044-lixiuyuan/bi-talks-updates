...
async function translateWithDeepL(text, target, source = "auto", timeoutMs = 18e4) {
  const key = readDeepLKey();
...
class TranslationScheduler {
  ...
  run(work, priority = 0) {
    ...
    return new Promise((resolve2, reject) => {
      this.queue.push({ priority, sequence: this.sequence++, run: async () => {
        try {
          resolve2(await work());
        } catch (error) {
          reject(error);
  ...
  async translate(text, target, source = "auto", _onDelta, options = {}) {
    ...
    const task = (async () => {
      ...
      return this.scheduler.run(async () => {
        ...
        if ((this.failedUntil.get(key) || 0) > Date.now()) throw new Error("上次翻译未完成，请稍后重试。");
        if (!readDeepLKey()) throw new Error("请先在设置中填写并保存 DeepL API Key。");
        let rawResult;
        try {
          rawResult = await translateWithDeepL(text, targetLanguage, sourceLanguage, options.timeoutMs);
        } catch (error) {
          this.failedUntil.set(key, Date.now() + 3e4);
...
function installSignalTranslation() {
  ...
  if (signalWindow.__biTalksSignalTranslation?.version === 7) return;
  ...
  const discardPending = (id, retry) => {
    ...
    if (attempts < 2) setTimeout(scheduleScan, 700);
      ...
      insertTranslation(job.element, job.key, "翻译暂未完成，点击重试");
  ...
  const request = (element) => {
    ...
    const timeout = setTimeout(() => discardPending(id, true), 52e3);
