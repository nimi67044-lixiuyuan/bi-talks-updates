...
function isPredominantlyChinese(text) {
  const han = count(text, hanPattern);
  if (!han) return false;
  if (count(text, kanaPattern) || count(text, hangulPattern)) return false;
  return true;
}
...
function createGroqApiKeyId() {
  return typeof globalThis.crypto?.randomUUID === "function" ? globalThis.crypto.randomUUID() : `groq-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`;
}
const platforms = {
  signal: { name: "Signal", color: "#3a76f0", iconSrc: signalIcon },
  whatsapp: { name: "WhatsApp", color: "#25d366" },
  telegram: { name: "Telegram", color: "#229ed9" },
  googlevoice: { name: "Google Voice", color: "#4285f4" }
};
function PlatformIcon({ platform, className = "platform-icon" }) {
  const [failed, setFailed] = reactExports.useState(false);
  reactExports.useEffect(() => setFailed(false), [platform]);
  if (platform === "whatsapp") {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `${className} platform-icon-fallback whatsapp-icon`, "aria-hidden": "true", children: /* @__PURE__ */ jsxRuntimeExports.jsx(MessageCircle, { size: 20 }) });
  }
  if (platform === "telegram") {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `${className} platform-icon-fallback telegram-icon`, "aria-hidden": "true", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Send, { size: 16 }) });
  }
  if (platform === "googlevoice") {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `${className} platform-icon-fallback googlevoice-icon`, "aria-hidden": "true", children: /* @__PURE__ */ jsxRuntimeExports.jsx(PhoneCall, { size: 17 }) });
  }
  if (failed || !platforms[platform].iconSrc) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx(
      "span",
      {
        className: `${className} platform-icon-fallback`,
        style: { background: platforms[platform].color },
        "aria-hidden": "true",
        children: platforms[platform].name.slice(0, 1).toUpperCase()
      }
    );
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsx("img", { className, src: platforms[platform].iconSrc, alt: "", "aria-hidden": "true", onError: () => setFailed(true) });
}
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    ...
    availablePatch && confirmationOpen && /* @__PURE__ */ jsxRuntimeExports.jsx(Modal, { title: "发现在线补丁", onClose: closeConfirmation, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "form-stack", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "confirm-copy", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: "发现新补丁" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: "确认更新后将下载并验证补丁。验证成功后，软件会退出、完成替换并自动重启。" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "patch-summary", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
          "目标版本 ",
          /* @__PURE__ */ jsxRuntimeExports.jsx("b", { children: availablePatch.targetVersion })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
          "补丁大小 ",
          /* @__PURE__ */ jsxRuntimeExports.jsx("b", { children: patchSize })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
          "发布时间 ",
          /* @__PURE__ */ jsxRuntimeExports.jsx("b", { children: new Date(availablePatch.publishedAt).toLocaleString() })
        ] })
      ] }),
      availablePatch.description && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "help-card", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
...
const webPlatforms = /* @__PURE__ */ new Set(["whatsapp", "telegram", "googlevoice"]);
const signalDesktopSupported = window.desktop.platform === "win32";
const translationPreferencePlatforms = /* @__PURE__ */ new Set(["signal", "whatsapp"]);
const translationTogglePlatforms = /* @__PURE__ */ new Set(["signal", "whatsapp"]);
const themeOptions = [
...
function applyAppearance(mode) {
  ...
  void window.desktop?.setNativeAppearanceEffects(false).catch(() => void 0);
}
function SortableItem({ id, children }) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id });
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { ref: setNodeRef, style: { transform: CSS.Transform.toString(transform), transition, opacity: isDragging ? 0.55 : 1 }, ...attributes, ...listeners, children });
}
function Modal({ title, children, onClose }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "modal-backdrop", onMouseDown: onClose, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal", onMouseDown: (event) => event.stopPropagation(), children: [
...
function shouldCollapseMessage(message) {
  const joined = `${message.originalText || ""}
${message.translatedText || ""}`.trim();
  return joined.length > 520 || joined.split(/\r?\n/).length > 10;
}
...
function conversationTitle(id) {
  if (!id || id === "default") return "New conversation";
  return id;
}
function unreadLabel(count2) {
  return count2 > 99 ? "99+" : String(count2);
}
function accountCardStatus(value) {
  const status = value.trim();
  return /^(?:ready|.+(?:\swindow)?\sready)$/i.test(status) ? void 0 : status;
}
function setAccountUnread(prev, accountId, count2) {
  const nextCount = Math.max(0, Math.min(999, Math.round(count2)));
  if (nextCount <= 0) {
  ...
  if (prev[accountId] === nextCount) return prev;
  return { ...prev, [accountId]: nextCount };
}
function incrementAccountUnread(prev, accountId) {
  return setAccountUnread(prev, accountId, (prev[accountId] || 0) + 1);
}
function applyExternalUnread(prev, accountId, count2) {
  const nextCount = Math.max(0, Math.min(999, Math.round(count2)));
  if (nextCount <= 0) return setAccountUnread(prev, accountId, 0);
  return setAccountUnread(prev, accountId, nextCount);
}
function summarizeConversations(messages, accountId, draftId = "") {
  if (!accountId) return [];
  ...
  for (const message of messages) {
    ...
    byId.set(message.conversationId, {
      ...
      title: conversationTitle(message.conversationId),
      lastText: message.translatedText || message.originalText || "Attachment",
      timestamp: message.timestamp
    });
  }
  const typed = draftId.trim();
  if (typed && !byId.has(typed)) {
    byId.set(typed, {
      id: typed,
      title: conversationTitle(typed),
      lastText: "Ready to send",
      timestamp: 0
    });
  }
  return [...byId.values()].sort((a, b) => b.timestamp - a.timestamp || a.title.localeCompare(b.title));
}
function MessageBubble({ message, refreshing, onRefresh }) {
  const [expanded, setExpanded] = reactExports.useState(false);
  const collapsed = shouldCollapseMessage(message);
  const originalText = collapsed && !expanded ? clampText(message.originalText) : message.originalText;
  const translatedText = message.translatedText ? collapsed && !expanded ? clampText(message.translatedText) : message.translatedText : void 0;
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bubble", children: [
    originalText && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "message-text", children: originalText }),
    translatedText && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "translation", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(Languages, { size: 14 }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: translatedText }),
      message.direction === "incoming" && /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          className: "translation-refresh",
          type: "button",
          title: "Refresh translation",
          "aria-label": "Refresh translation",
          disabled: refreshing,
          onClick: () => onRefresh(message),
          children: /* @__PURE__ */ jsxRuntimeExports.jsx(RotateCw, { className: refreshing ? "spin" : "", size: 12 })
        }
      )
    ] }),
    message.attachmentNames?.map((name) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "attachment-chip", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(Paperclip, { size: 13 }),
      name
    ] }, name)),
    collapsed && /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "read-more", type: "button", onClick: () => setExpanded((value) => !value), children: expanded ? "收起" : "阅读全文" })
  ] });
}
function App() {
  ...
  const mainPanels = reactExports.useRef(null);
  const accountPanel = reactExports.useRef(null);
  const contentArea = reactExports.useRef(null);
  const webSurface = reactExports.useRef(null);
  const signalSurface = reactExports.useRef(null);
  const stopAccountResize = reactExports.useRef(void 0);
  const knownAccountIds = reactExports.useRef(/* @__PURE__ */ new Set());
  const latestState = reactExports.useRef(void 0);
  const seenMessageIds = reactExports.useRef(/* @__PURE__ */ new Set());
  const activeUnreadTarget = reactExports.useRef({ conversationId: "default" });
  const unreadEventRevision = reactExports.useRef(0);
  reactExports.useEffect(() => {
    const visibleAccountIds = new Set((state?.accounts || []).filter((account) => account.platform !== "telegram").map((account) => account.id));
  ...
  const webResizeFrame = reactExports.useRef(void 0);
  const telegramResizeBoundsTimer = reactExports.useRef(void 0);
  const telegramSidebarResizeActive = reactExports.useRef(false);
  const telegramNativeViewSuspended = reactExports.useRef(false);
  const telegramResizeAccountId = reactExports.useRef(void 0);
  const telegramResizeSnapshotRequestId = reactExports.useRef(0);
  const pendingWebBounds = reactExports.useRef(void 0);
  const lastSentWebBounds = reactExports.useRef(void 0);
  const panelResizeFrame = reactExports.useRef(void 0);
  const pendingPanelWidth = reactExports.useRef(void 0);
  const signalResizeFrame = reactExports.useRef(void 0);
  const pendingSignalPlacement = reactExports.useRef(void 0);
  const activeSignalAccountId = reactExports.useRef(void 0);
  const signalOverlayUiOpen = reactExports.useRef(false);
  const signalAttachRunId = reactExports.useRef(0);
  const [attachedSignalAccountIds, setAttachedSignalAccountIds] = reactExports.useState(() => /* @__PURE__ */ new Set());
  const sensors = useSensors(useSensor(PointerSensor, { activationConstraint: { distance: 5 } }));
  const save = reactExports.useCallback(async (next) => {
    setState(next);
    await window.desktop.setState(next);
  }, []);
  reactExports.useEffect(() => {
    let disposed = false;
    const unsubscribeNoticeDismissed = window.desktop.onWorkspaceNoticeDismissed(() => {
      setNotice(void 0);
    ...
    const unsubscribe = window.desktop.onEvent((event) => {
      ...
      if (event.type === "message") {
        const messageKey = `${event.message.accountId}:${event.message.id}`;
        const isNewMessage = !seenMessageIds.current.has(messageKey);
        if (isNewMessage) {
          seenMessageIds.current.add(messageKey);
          if (seenMessageIds.current.size > 1600) {
            for (const key of seenMessageIds.current) {
              seenMessageIds.current.delete(key);
              if (seenMessageIds.current.size <= 1200) break;
            }
          }
        }
        const messagePlatform = latestState.current?.accounts.find((account) => account.id === event.message.accountId)?.platform;
        if (isNewMessage && event.message.direction === "incoming" && messagePlatform !== "telegram") {
          const activeTarget = activeUnreadTarget.current;
          const isVisibleThread = document.hasFocus() && activeTarget.accountId === event.message.accountId && activeTarget.conversationId === event.message.conversationId;
          if (!isVisibleThread) {
            unreadEventRevision.current += 1;
            setUnreadCounts((items) => incrementAccountUnread(items, event.message.accountId));
          }
        }
        setMessages((items) => {
          const duplicateIndex = items.findIndex((item) => item.id === event.message.id && item.accountId === event.message.accountId);
          if (duplicateIndex >= 0) {
            const next = [...items];
            next[duplicateIndex] = event.message;
            return next;
          }
          return [...items.slice(-499), event.message];
        });
      }
      if (event.type === "connection") {
        setStatuses((items) => ({ ...items, [event.accountId]: event.status }));
        const connectionStatus = event.status.toLowerCase();
        if (connectionStatus.includes("managed signal closed")) {
          autoSignalAttachAttempts.current.delete(event.accountId);
          setAttachedSignalAccountIds((items) => {
            if (!items.has(event.accountId)) return items;
            const next = new Set(items);
            next.delete(event.accountId);
            return next;
          });
        }
        if (connectionStatus.includes("signal window ready")) {
          const signalAccount = latestState.current?.accounts.find((item) => item.id === event.accountId && item.platform === "signal");
          if (signalAccount && activeSignalAccountId.current !== event.accountId) {
            void window.desktop.setSignalWindowVisible(signalAccount, false).catch(() => void 0);
          }
          setAttachedSignalAccountIds((items) => {
            if (items.has(event.accountId)) return items;
            const next = new Set(items);
      ...
      if (event.type === "unread") {
        unreadEventRevision.current += 1;
        if (latestState.current?.accounts.some((account) => account.id === event.accountId && account.platform === "telegram")) {
          setUnreadCounts((items) => applyExternalUnread(items, event.accountId, 0));
          return;
        }
        setUnreadCounts((items) => applyExternalUnread(items, event.accountId, event.count));
      }
      if (event.type === "workspace-preview") {
        setWorkspacePreview((current) => current?.accountId === event.accountId && current.dataUrl === event.dataUrl ? current : { accountId: event.accountId, dataUrl: event.dataUrl });
      }
      if (event.type === "signal-update") {
        const progress = typeof event.progress === "number" && event.status === "downloading" ? Math.min(100, Math.max(0, event.progress)) : void 0;
        const message = event.message;
        setNotice(message);
        setNoticeProgress(progress);
        if (event.status === "checking" || event.status === "downloading" || event.status === "installing") {
          setSignalUpdating(true);
        } else {
          setSignalUpdating(false);
        }
        if (event.accountId) setStatuses((items) => ({ ...items, [event.accountId]: message }));
      }
      if (event.type === "app-patch") {
        if (event.status === "checking" || event.status === "available" || event.status === "downloading" || event.status === "ready" || event.status === "applying" || event.status === "up-to-date" || event.status === "error") {
          setNotice(void 0);
          setNoticeProgress(void 0);
        } else {
      ...
      if (event.type === "signal-translation" && event.status === "error") {
        setNotice(event.message);
        setStatuses((items) => ({ ...items, [event.accountId]: "翻译连接失败" }));
      }
      if (event.type === "error") {
        setNotice(event.message);
        if (event.accountId) setStatuses((items) => ({ ...items, [event.accountId]: "Connection failed" }));
      }
    });
    void window.desktop.getState().then((next) => {
      if (!disposed) setState(next);
    });
    const unreadSnapshotRevision = unreadEventRevision.current;
    void window.desktop.getUnreadCounts().then((snapshot) => {
      if (!disposed && unreadEventRevision.current === unreadSnapshotRevision) setUnreadCounts(snapshot);
    });
    return () => {
      disposed = true;
      unsubscribe();
      unsubscribeNoticeDismissed();
    };
  }, []);
  reactExports.useEffect(() => {
    let disposed = false;
    void window.desktop.isSignalDesktopUpdating().then((active) => {
      if (!disposed) setSignalUpdating(active);
    }).catch(() => {
      if (!disposed) setSignalUpdating(false);
    });
    return () => {
      disposed = true;
    };
  ...
  reactExports.useEffect(() => {
    const nextIds = new Set(state?.accounts.map((account) => account.id) || []);
    knownAccountIds.current = nextIds;
    setUnreadCounts((items) => {
      let changed = false;
      const next = { ...items };
  ...
  reactExports.useEffect(() => {
    if (!state) return;
    applyAppearance(state.settings.themeMode);
    if (state.settings.themeMode !== "system") return;
    const media = window.matchMedia("(prefers-color-scheme: light)");
    const update = () => applyAppearance(state.settings.themeMode);
  ...
  const accounts = reactExports.useMemo(() => {
    if (!state) return [];
    const ids2 = state.accountOrder[state.selectedPlatform] || [];
    const byId = new Map(state.accounts.filter((item) => item.platform === state.selectedPlatform && !(item.platform === "telegram" && item.hidden)).map((item) => [item.id, item]));
    return ids2.map((id) => byId.get(id)).filter(Boolean);
  }, [state]);
  ...
  const activeConversationId = conversationId.trim() || conversations[0]?.id || "default";
  reactExports.useEffect(() => {
    activeUnreadTarget.current = { accountId: selectedAccount?.id, conversationId: activeConversationId };
  }, [selectedAccount?.id, activeConversationId]);
  const webBounds = reactExports.useCallback(() => {
    const rect = webSurface.current?.getBoundingClientRect();
  ...
  const updateBounds = reactExports.useCallback(() => {
    const bounds = webBounds();
    if (!bounds) return Promise.resolve();
    const previous = lastSentWebBounds.current;
  ...
  reactExports.useCallback((panelWidth) => {
    ...
    const x = Math.round(panels.left + railWidth + panelWidth + resizeHandleWidth);
    const width = Math.max(80, Math.round(panels.width - railWidth - panelWidth - resizeHandleWidth));
    return {
      x,
  ...
  const moveWebBoundsNow = reactExports.useCallback((forcedBounds) => {
    const bounds = forcedBounds || webBounds();
    if (!bounds) return;
    const previous = lastSentWebBounds.current;
    if (previous && previous.x === bounds.x && previous.y === bounds.y && previous.width === bounds.width && previous.height === bounds.height) return;
    lastSentWebBounds.current = bounds;
    void window.desktop.setViewBounds(bounds).catch(() => void 0);
  }, [webBounds]);
  const scheduleWebBoundsSync = reactExports.useCallback((forcedBounds) => {
    if (telegramNativeViewSuspended.current) return;
    pendingWebBounds.current = forcedBounds;
    if (telegramSidebarResizeActive.current) {
      if (telegramResizeBoundsTimer.current !== void 0) return;
      telegramResizeBoundsTimer.current = window.setTimeout(() => {
        telegramResizeBoundsTimer.current = void 0;
        const bounds = pendingWebBounds.current;
        pendingWebBounds.current = void 0;
        moveWebBoundsNow(bounds);
      }, 80);
      return;
    }
    if (webResizeFrame.current !== void 0) return;
    webResizeFrame.current = window.requestAnimationFrame(() => {
      webResizeFrame.current = void 0;
      const bounds = pendingWebBounds.current;
      pendingWebBounds.current = void 0;
      moveWebBoundsNow(bounds);
    });
  }, [moveWebBoundsNow]);
  const signalBounds = reactExports.useCallback(() => {
    const rect = signalSurface.current?.getBoundingClientRect();
    if (!rect) return void 0;
    return {
      x: Math.round(rect.x),
      y: Math.round(rect.y),
  ...
  const syncSignalBounds = reactExports.useCallback((visible = true) => {
    if (!selectedAccount || selectedAccount.platform !== "signal" || !selectedSignalIsAttached) return;
    if (visible && signalOverlayUiOpen.current) return;
    if (visible && activeSignalAccountId.current !== selectedAccount.id) return;
    const bounds = signalBounds();
    if (bounds) void window.desktop.setSignalWindowVisible(selectedAccount, visible, bounds).catch(() => void 0);
  }, [selectedAccount, selectedSignalIsAttached, signalBounds]);
  const moveSignalBoundsNow = reactExports.useCallback((visible = true, forcedBounds) => {
    if (!selectedAccount || selectedAccount.platform !== "signal" || !selectedSignalIsAttached) return;
    if (visible && signalOverlayUiOpen.current) return;
    if (visible && activeSignalAccountId.current !== selectedAccount.id) return;
    const bounds = forcedBounds || signalBounds();
    if (!bounds) return;
    window.desktop.moveSignalWindow(selectedAccount, visible, bounds);
  }, [selectedAccount, selectedSignalIsAttached, signalBounds]);
  const scheduleSignalBoundsSync = reactExports.useCallback((visible = true, forcedBounds) => {
    pendingSignalPlacement.current = { visible, bounds: forcedBounds };
    if (signalResizeFrame.current !== void 0) return;
    signalResizeFrame.current = window.requestAnimationFrame(() => {
      signalResizeFrame.current = void 0;
      const placement = pendingSignalPlacement.current;
      pendingSignalPlacement.current = void 0;
      moveSignalBoundsNow(placement?.visible ?? visible, placement?.bounds);
    });
  }, [moveSignalBoundsNow]);
  const hideSignalForOverlayUi = reactExports.useCallback(() => {
    if (!selectedAccount || selectedAccount.platform !== "signal" || !selectedSignalIsAttached) return;
    signalOverlayUiOpen.current = true;
    const bounds = signalBounds();
    void window.desktop.setSignalWindowVisible(selectedAccount, false, bounds).catch(() => void 0);
  }, [selectedAccount, selectedSignalIsAttached, signalBounds]);
  const openAddAccount = reactExports.useCallback(() => {
  ...
  reactExports.useEffect(() => {
    const accountId = selectedAccount?.platform === "signal" ? selectedAccount.id : void 0;
    if (activeSignalAccountId.current === accountId) return;
    activeSignalAccountId.current = accountId;
    window.desktop.setActiveSignalAccount(accountId);
  }, [selectedAccount?.id, selectedAccount?.platform]);
  reactExports.useEffect(() => {
    const node = webSurface.current;
    if (!node) return;
    const observer = new ResizeObserver(() => scheduleWebBoundsSync());
    observer.observe(node);
    scheduleWebBoundsSync();
    return () => observer.disconnect();
  }, [scheduleWebBoundsSync, selectedAccount?.id]);
  reactExports.useEffect(() => {
    const syncNoticeOverlay = () => {
      const rect = contentArea.current?.getBoundingClientRect();
      window.desktop.setWorkspaceNotice(notice, rect ? {
        x: Math.round(rect.x),
        y: Math.round(rect.y),
        width: Math.round(rect.width),
        height: Math.round(rect.height)
      } : void 0, noticeProgress);
    };
    syncNoticeOverlay();
    if (!notice || !contentArea.current) return;
    const observer = new ResizeObserver(syncNoticeOverlay);
    observer.observe(contentArea.current);
    window.addEventListener("resize", syncNoticeOverlay);
    return () => {
      observer.disconnect();
      window.removeEventListener("resize", syncNoticeOverlay);
      window.desktop.setWorkspaceNotice(void 0);
    };
  }, [notice, noticeProgress]);
  reactExports.useEffect(() => {
    const node = signalSurface.current;
    if (!node || !selectedAccount || selectedAccount.platform !== "signal" || !selectedSignalIsAttached) return;
    const observer = new ResizeObserver(() => {
      scheduleSignalBoundsSync(true);
    });
    observer.observe(node);
    return () => observer.disconnect();
  }, [selectedAccount?.id, selectedSignalIsAttached, scheduleSignalBoundsSync]);
  reactExports.useEffect(() => {
    if (!modalIsOpen) signalOverlayUiOpen.current = false;
  }, [modalIsOpen]);
  reactExports.useEffect(() => {
    if (!selectedAccount || selectedAccount.platform !== "signal" || !selectedSignalIsAttached) return;
    syncSignalBounds(!modalIsOpen);
  }, [modalIsOpen, selectedAccount?.id, selectedSignalIsAttached, syncSignalBounds]);
  reactExports.useEffect(() => {
    if (!selectedAccount || selectedAccount.platform !== "signal" || !selectedSignalIsAttached) return;
    if (modalIsOpen) return;
    const syncOnFocus = () => syncSignalBounds(true);
    window.addEventListener("focus", syncOnFocus);
    return () => window.removeEventListener("focus", syncOnFocus);
  }, [modalIsOpen, selectedAccount?.id, selectedSignalIsAttached, syncSignalBounds]);
  reactExports.useEffect(() => {
    if (!signalDesktopSupported) return;
    if (!selectedAccount || selectedAccount.platform !== "signal") return;
    if (selectedSignalIsAttached) return;
    if (modalIsOpen || signalUpdating) return;
    const bounds = signalBounds();
    if (!bounds || autoSignalAttachAttempts.current.has(selectedAccount.id) || manuallyClosedAccountIds.current.has(selectedAccount.id)) return;
    const attachRunId = signalAttachRunId.current;
    const account = selectedAccount;
    autoSignalAttachAttempts.current.add(selectedAccount.id);
    setNotice(void 0);
    setStatuses((items) => ({ ...items, [selectedAccount.id]: "Launching managed Signal" }));
    void window.desktop.attachSignalWindow(account, bounds).then(() => {
      if (signalAttachRunId.current !== attachRunId || activeSignalAccountId.current !== account.id) {
        void window.desktop.setSignalWindowVisible(account, false).catch(() => void 0);
        return;
      }
      setNotice(void 0);
      setAttachedSignalAccountIds((items) => {
      ...
      if (signalAttachRunId.current !== attachRunId || activeSignalAccountId.current !== account.id) return;
      setNotice(error instanceof Error ? error.message : String(error));
      setStatuses((items) => ({ ...items, [account.id]: "Signal launch failed" }));
      autoSignalAttachAttempts.current.delete(account.id);
    });
  }, [accountLifecycleRevision, modalIsOpen, selectedAccount?.id, selectedSignalIsAttached, signalBounds, signalUpdating]);
  reactExports.useEffect(() => {
    let cancelled = false;
    void (async () => {
      try {
        if (modalIsOpen && workspaceIsWeb) {
          await window.desktop.activateView(void 0);
          return;
        }
        if (selectedAccount && manuallyClosedAccountIds.current.has(selectedAccount.id)) {
          await window.desktop.activateView(void 0);
          return;
        }
        if (selectedAccount && webPlatforms.has(selectedAccount.platform)) await updateBounds();
        if (cancelled) return;
        await window.desktop.activateView(selectedAccount);
      } catch (error) {
  ...
  }, [accountLifecycleRevision, modalIsOpen, selectedAccount, updateBounds, workspaceIsWeb]);
  reactExports.useEffect(() => {
    if (!selectedAccount || webPlatforms.has(selectedAccount.platform) || selectedAccount.platform === "signal") return;
    void window.desktop.startAccount(selectedAccount);
  }, [selectedAccount?.id]);
  reactExports.useEffect(() => {
    if (!selectedAccount || workspaceIsWeb || conversationId.trim() || !conversations[0]) return;
    setConversationId(conversations[0].id);
  }, [selectedAccount?.id, workspaceIsWeb, conversationId, conversations]);
  reactExports.useEffect(() => {
    return () => {
      stopAccountResize.current?.();
      if (webResizeFrame.current !== void 0) window.cancelAnimationFrame(webResizeFrame.current);
      pendingWebBounds.current = void 0;
      ...
      if (signalResizeFrame.current !== void 0) window.cancelAnimationFrame(signalResizeFrame.current);
      pendingSignalPlacement.current = void 0;
    };
  }, []);
  reactExports.useEffect(() => {
    if (signalResizeFrame.current !== void 0) {
      window.cancelAnimationFrame(signalResizeFrame.current);
      signalResizeFrame.current = void 0;
    }
    pendingSignalPlacement.current = void 0;
  }, [selectedAccount?.id]);
  const selectPlatform = async (platform) => {
    ...
    const rememberedAccountId = state.selectedAccountIdsByPlatform[platform];
    const visibleOnPlatform = (item) => item.platform === platform && !(platform === "telegram" && item.hidden);
    const selectedAccountId = rememberedAccountId && state.accounts.some((item) => item.id === rememberedAccountId && visibleOnPlatform(item)) ? rememberedAccountId : state.accountOrder[platform]?.find((id) => state.accounts.some((item) => item.id === id && visibleOnPlatform(item)));
    const nextAccount = state.accounts.find((item) => item.id === selectedAccountId);
    const nextSignalAccountId = nextAccount?.platform === "signal" ? nextAccount.id : void 0;
    const selectedAccountIdsByPlatform = { ...state.selectedAccountIdsByPlatform };
    if (selectedAccountId) selectedAccountIdsByPlatform[platform] = selectedAccountId;
    else delete selectedAccountIdsByPlatform[platform];
    activeSignalAccountId.current = nextSignalAccountId;
    window.desktop.setActiveSignalAccount(nextSignalAccountId);
    setConversationId("");
    await save({ ...state, selectedPlatform: platform, selectedAccountId, selectedAccountIdsByPlatform });
  };
  const selectAccount = async (account) => {
    if (!state) return;
    const switchRunId = signalAttachRunId.current + 1;
    signalAttachRunId.current = switchRunId;
    const previousAccount = selectedAccount;
    const nextSignalAccountId = account.platform === "signal" ? account.id : void 0;
    const targetIsAttachedSignal = account.platform === "signal" && attachedSignalAccountIds.has(account.id);
    const previousWasAttachedSignal = previousAccount?.platform === "signal" && previousAccount.id !== account.id && attachedSignalAccountIds.has(previousAccount.id);
    const nextState = {
      ...state,
      selectedPlatform: account.platform,
      selectedAccountId: account.id,
      selectedAccountIdsByPlatform: {
        ...state.selectedAccountIdsByPlatform,
        [account.platform]: account.id
      }
    };
    activeSignalAccountId.current = nextSignalAccountId;
    window.desktop.setActiveSignalAccount(nextSignalAccountId, targetIsAttachedSignal && previousWasAttachedSignal);
    setConversationId("");
    setState(nextState);
    const persistSelection = window.desktop.setState(nextState);
    void persistSelection.catch((error) => {
      if (signalAttachRunId.current === switchRunId) setNotice(error instanceof Error ? error.message : String(error));
    });
    void (async () => {
      if (targetIsAttachedSignal) {
        const bounds = signalBounds();
        try {
          await window.desktop.setSignalWindowVisible(account, true, bounds);
        } catch (error) {
          if (signalAttachRunId.current !== switchRunId) return;
          if (previousWasAttachedSignal && previousAccount) {
            const rollbackState = {
              ...nextState,
              selectedPlatform: previousAccount.platform,
              selectedAccountId: previousAccount.id,
              selectedAccountIdsByPlatform: {
                ...nextState.selectedAccountIdsByPlatform,
                [previousAccount.platform]: previousAccount.id
              }
            };
            activeSignalAccountId.current = previousAccount.id;
            window.desktop.setActiveSignalAccount(previousAccount.id);
            setState(rollbackState);
            await persistSelection.catch(() => void 0);
            await window.desktop.setState(rollbackState).catch(() => void 0);
            await window.desktop.setSignalWindowVisible(previousAccount, true, bounds).catch(() => void 0);
          } else {
            setAttachedSignalAccountIds((items) => {
              if (!items.has(account.id)) return items;
              const next = new Set(items);
              next.delete(account.id);
              return next;
            });
          }
          setNotice(`Signal 账号切换失败：${error instanceof Error ? error.message : String(error)}`);
          return;
        }
        if (signalAttachRunId.current !== switchRunId) return;
        setNotice(void 0);
      }
      if (previousWasAttachedSignal && signalAttachRunId.current === switchRunId) {
        await window.desktop.setSignalWindowVisible(previousAccount, false).catch(() => void 0);
      }
    })();
  };
  ...
  const startAccountAction = async (account) => {
    ...
    if (webPlatforms.has(account.platform)) {
      setStatuses((items) => ({ ...items, [account.id]: "Starting" }));
      await window.desktop.startAccount(account).then(() => setStatuses((items) => ({ ...items, [account.id]: "Ready" }))).catch((error) => {
        setStatuses((items) => ({ ...items, [account.id]: "Start failed" }));
        setNotice(error instanceof Error ? error.message : String(error));
      });
    }
    if (account.platform === "signal" && signalDesktopSupported) {
      autoSignalAttachAttempts.current.delete(account.id);
      const bounds = signalBounds();
      if (bounds) {
        const attachRunId = signalAttachRunId.current;
        autoSignalAttachAttempts.current.add(account.id);
        setStatuses((items) => ({ ...items, [account.id]: "Launching managed Signal" }));
        await window.desktop.attachSignalWindow(account, bounds).then(() => {
          if (signalAttachRunId.current !== attachRunId || activeSignalAccountId.current !== account.id) {
            void window.desktop.setSignalWindowVisible(account, false).catch(() => void 0);
            return;
          }
          setNotice(void 0);
          setAttachedSignalAccountIds((items) => new Set(items).add(account.id));
        }).catch((error) => {
          if (signalAttachRunId.current !== attachRunId || activeSignalAccountId.current !== account.id) return;
          autoSignalAttachAttempts.current.delete(account.id);
          setNotice(error instanceof Error ? error.message : String(error));
  ...
  const refreshAccountAction = async (account) => {
    ...
    setStatuses((items) => ({ ...items, [account.id]: "Refreshing" }));
    await window.desktop.refreshAccount(account).then(() => setStatuses((items) => ({ ...items, [account.id]: "Ready" }))).catch((error) => {
      setStatuses((items) => ({ ...items, [account.id]: "Refresh failed" }));
      setNotice(error instanceof Error ? error.message : String(error));
    }).finally(() => {
  ...
  const closeAccountAction = async (account) => {
    ...
    await window.desktop.closeAccount(account).then(() => setStatuses((items) => ({ ...items, [account.id]: "Closed" }))).catch((error) => {
      manuallyClosedAccountIds.current.delete(account.id);
      setAccountLifecycleRevision((value) => value + 1);
      setStatuses((items) => ({ ...items, [account.id]: "Close failed" }));
      setNotice(error instanceof Error ? error.message : String(error));
    });
  };
  const updateSignalDesktopAction = async (account) => {
    ...
    if (updateAlreadyRunning) {
      setSignalUpdating(true);
      setNotice("Signal 更新正在进行，请稍候");
      return;
    }
    setSignalUpdating(false);
    setSignalUpdating(true);
    setStatuses((items) => ({ ...items, [account.id]: "正在更新 Signal" }));
    try {
      const result = await window.desktop.updateSignalDesktop(account);
      setNotice(result.updated ? `Signal 已更新到 ${result.version}` : `Signal 已经是最新版本 ${result.version}`);
    } catch (error) {
      setNotice(error instanceof Error ? error.message : String(error));
    } finally {
      setAccountLifecycleRevision((value) => value + 1);
  ...
  const platformDrop = async ({ active, over }) => {
    if (!state || !over || active.id === over.id) return;
    const oldIndex = state.platformOrder.indexOf(active.id);
    const newIndex = state.platformOrder.indexOf(over.id);
    if (oldIndex < 0 || newIndex < 0) return;
    await save({ ...state, platformOrder: arrayMove(state.platformOrder, oldIndex, newIndex) });
  };
  const accountDrop = async ({ active, over }) => {
    if (!state || !over || active.id === over.id) return;
    const order = state.accountOrder[state.selectedPlatform] || [];
    const oldIndex = order.indexOf(String(active.id));
    const newIndex = order.indexOf(String(over.id));
    if (oldIndex < 0 || newIndex < 0) return;
    await save({
      ...state,
      accountOrder: { ...state.accountOrder, [state.selectedPlatform]: arrayMove(order, oldIndex, newIndex) }
    });
  };
  const removeAccount = async (account) => {
    if (!state) return;
    const accountsLeft = state.accounts.filter((item) => item.id !== account.id);
    const order = state.accountOrder[account.platform].filter((id) => id !== account.id);
    const firstVisibleAccountId = (platform, accountIds) => accountIds.find((id) => {
      const candidate = accountsLeft.find((item) => item.id === id);
      return candidate?.platform === platform && !(platform === "telegram" && candidate.hidden);
    });
    const nextVisibleAccountId = firstVisibleAccountId(account.platform, order);
    const selectedAccountIdsByPlatform = { ...state.selectedAccountIdsByPlatform };
    if (selectedAccountIdsByPlatform[account.platform] === account.id) {
      if (nextVisibleAccountId) selectedAccountIdsByPlatform[account.platform] = nextVisibleAccountId;
      else delete selectedAccountIdsByPlatform[account.platform];
    }
    const selectedAccountId = state.selectedAccountId === account.id ? account.platform === state.selectedPlatform ? nextVisibleAccountId : selectedAccountIdsByPlatform[state.selectedPlatform] : state.selectedAccountId;
    autoSignalAttachAttempts.current.delete(account.id);
    manuallyClosedAccountIds.current.delete(account.id);
    setAccountLifecycleRevision((value) => value + 1);
    setUnreadCounts((items) => setAccountUnread(items, account.id, 0));
    setAttachedSignalAccountIds((items) => {
      if (!items.has(account.id)) return items;
      const next = new Set(items);
      next.delete(account.id);
    ...
    await window.desktop.removeAccount(account);
    ...
    await save({
      ...
      selectedAccountId,
      selectedAccountIdsByPlatform,
      accountOrder: { ...state.accountOrder, [account.platform]: order }
    });
    setStatuses((items) => {
      const next = { ...items };
  ...
  const refreshTranslation = async (message) => {
    ...
    setRefreshingMessageIds((items) => new Set(items).add(key));
    setNotice(void 0);
    try {
      const translatedText = await window.desktop.translate(
        message.originalText,
        bilingualTranslationTarget(message.originalText)
      );
      setMessages((items) => items.map((item) => item.id === message.id && item.accountId === message.accountId ? { ...item, translatedText } : item));
    } catch (error) {
      setNotice(error instanceof Error ? error.message : String(error));
    } finally {
      setRefreshingMessageIds((items) => {
        const next = new Set(items);
        next.delete(key);
        return next;
      });
  ...
  for (const account of state.accounts) {
    ...
    platformUnreadCounts[account.platform] += unreadCounts[account.id] || 0;
  }
  const workspaceTranslationStyle = selectedAccount ? {
    "--bi-talks-translation-color": selectedAccount.translationColor || "#d8ff00",
    "--bi-talks-translation-size": `${selectedAccount.translationSize || 15}px`
  } : void 0;
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "app-shell", children: [
      ...
      "div",
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("aside", { className: "rail-panel", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "platform-rail", children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(DndContext, { sensors, collisionDetection: closestCenter, onDragEnd: platformDrop, children: /* @__PURE__ */ jsxRuntimeExports.jsx(SortableContext, { items: visiblePlatformOrder, strategy: verticalListSortingStrategy, children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "platform-list", children: visiblePlatformOrder.map((platform) => {
              return /* @__PURE__ */ jsxRuntimeExports.jsx(SortableItem, { id: platform, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "button",
                {
                  className: `platform-button ${state.selectedPlatform === platform ? "active" : ""}`,
                  style: { "--brand": platforms[platform].color },
                  title: platforms[platform].name,
                  onClick: () => void selectPlatform(platform),
                  children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsx(PlatformIcon, { platform }),
                    !!platformUnreadCounts[platform] && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "unread-badge platform-unread-badge", children: unreadLabel(platformUnreadCounts[platform]) })
                  ]
                }
              ) }, platform);
            }) }) }) }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rail-bottom", style: { display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }, children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                UpdateCheckButton,
                {
                  onNotice: setNotice,
                  onDialogOpenChange: (open) => {
                    if (open) hideSignalForOverlayUi();
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("aside", { ref: accountPanel, className: "account-panel-shell", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "account-panel", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(DndContext, { sensors, collisionDetection: closestCenter, onDragEnd: accountDrop, children: /* @__PURE__ */ jsxRuntimeExports.jsx(SortableContext, { items: accounts.map((item) => item.id), strategy: horizontalListSortingStrategy, children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "account-list", children: accounts.map((account) => {
              const cardStatus = accountCardStatus(statuses[account.id] || account.identifier || "Ready");
              return /* @__PURE__ */ jsxRuntimeExports.jsx(SortableItem, { id: account.id, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "div",
                {
                  ...
                  children: [
                    ...
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `account-copy${cardStatus ? " has-status" : ""}`, children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: account.label }),
                      cardStatus && /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: cardStatus })
                    ] })
                  ]
                }
              ) }, account.id);
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("main", { className: "workspace-panel", children: /* @__PURE__ */ jsxRuntimeExports.jsx("section", { className: "workspace", style: workspaceTranslationStyle, children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { ref: contentArea, className: "content-area", children: workspaceIsWeb ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { ref: webSurface, className: "web-surface", children: workspacePreview?.accountId === selectedAccount?.id ? /* @__PURE__ */ jsxRuntimeExports.jsx("img", { className: "workspace-taskbar-preview", src: workspacePreview.dataUrl, alt: "", "aria-hidden": "true", draggable: false }) : /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "web-loading", children: [
            ...
            !selectedSignalLooksReady && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "signal-desktop-card", children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { children: signalDesktopSupported ? "Launching managed Signal" : "Signal integration is unavailable" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: signalDesktopSupported ? "This account starts a separate app-managed Signal Desktop instance with its own data directory. Existing Signal windows on your desktop are ignored." : "The managed Signal window requires the bundled Windows integration components." }),
              signalDesktopSupported && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "loading-inline", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(LoaderCircle, { className: "spin", size: 16 }),
                "Starting"
              ] })
            ] })
          ] }) : /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `chat-workspace ${selectedAccount ? "" : "no-account"}`, children: [
            selectedAccount && /* @__PURE__ */ jsxRuntimeExports.jsxs("aside", { className: "conversation-panel", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "conversation-panel-title", children: "Chats" }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "conversation-items", children: [
                conversations.map((conversation) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "button",
                  {
                    ...
                    children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "conversation-avatar", children: conversation.title.slice(0, 1).toUpperCase() }),
                      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "conversation-copy", children: [
                        /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: conversation.title }),
                        /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: conversation.lastText })
                      ] }),
                      !!conversation.timestamp && /* @__PURE__ */ jsxRuntimeExports.jsx("time", { children: new Date(conversation.timestamp).toLocaleTimeString() })
                    ]
                  },
                  conversation.id
                )),
                !conversations.length && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "conversation-hint", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx(MessagesSquare, { size: 22 }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "No chats yet" })
                ] })
              ] })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "message-list", children: [
              selectedAccount && filteredMessages.map((message) => /* @__PURE__ */ jsxRuntimeExports.jsxs("article", { className: `message ${message.direction}`, children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  MessageBubble,
                  {
                    message,
                    refreshing: refreshingMessageIds.has(`${message.accountId}:${message.id}`),
                    onRefresh: (item) => void refreshTranslation(item)
                  }
                ),
                /* @__PURE__ */ jsxRuntimeExports.jsx("time", { children: new Date(message.timestamp).toLocaleTimeString() })
              ] }, message.id)),
              !selectedAccount && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "conversation-empty", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(MessagesSquare, { size: 42 }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { children: "Unified translation chat" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: "Choose an account to start receiving and translating messages." })
              ] }),
              selectedAccount && !filteredMessages.length && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "conversation-empty", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(MessagesSquare, { size: 42 }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { children: "No messages yet" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: "Choose a chat to start." })
              ] })
            ] })
          ] }) }) }) })
        ]
    ...
    confirmRemove && /* @__PURE__ */ jsxRuntimeExports.jsx(
      ConfirmRemoveAccount,
      {
        ...
        onConfirm: async () => {
          try {
            await removeAccount(confirmRemove);
    ...
    confirmCloseOpen && /* @__PURE__ */ jsxRuntimeExports.jsx(
      ConfirmCloseApplication,
      {
        onCancel: () => setConfirmCloseOpen(false),
        onConfirm: () => window.desktop.confirmClose()
      }
    )
  ] });
}
function AddAccount({ state, save, onClose, onAdded }) {
  ...
  const telegramNameKey = (value) => value.trim().normalize("NFKC").toLocaleLowerCase();
  const submit = async () => {
    if (submitting) return;
    setSubmitting(true);
    setError("");
    try {
      const trimmedLabel = label.trim() || platformName;
      if (platform === "telegram" && label.trim()) {
        const existing = state.accounts.find((item) => item.platform === "telegram" && telegramNameKey(item.label) === telegramNameKey(label));
        if (existing) {
          const hidden = !existing.hidden;
          const updatedAccount = { ...existing, hidden };
          const accounts = state.accounts.map((item) => item.id === existing.id ? updatedAccount : item);
          const nextVisibleTelegramId = state.accountOrder.telegram.find((id) => {
            const candidate = accounts.find((item) => item.id === id);
            return candidate?.platform === "telegram" && !candidate.hidden;
          });
          const hidesSelectedTelegram = hidden && state.selectedPlatform === "telegram" && state.selectedAccountId === existing.id;
          const selectedAccountId = hidden ? hidesSelectedTelegram ? nextVisibleTelegramId : state.selectedAccountId : existing.id;
          const selectedAccountIdsByPlatform = { ...state.selectedAccountIdsByPlatform };
          if (selectedAccountId) selectedAccountIdsByPlatform.telegram = selectedAccountId;
          else delete selectedAccountIdsByPlatform.telegram;
          await save({
            ...state,
      ...
      await save({
        ...
        accountOrder: {
          ...state.accountOrder,
          [platform]: [...state.accountOrder[platform] || [], account.id]
        }
      });
      onAdded?.(account);
      setSubmitting(false);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(Modal, { title: `Add ${platformName} account`, onClose, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { children: [
          "Display name",
          /* @__PURE__ */ jsxRuntimeExports.jsx("input", { value: label, onChange: (event) => setLabel(event.target.value), placeholder: `${platformName} account` })
        ] }),
        platform === "googlevoice" && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "google-voice-add-note", children: "输入账号密码后请刷新账号以便使用" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-actions", children: [
          error && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "form-error", children: error }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "ghost-button", type: "button", disabled: submitting, onClick: onClose, children: "Cancel" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "primary-button", type: "button", disabled: submitting, onClick: () => void submit(), children: submitting ? "Saving..." : "Add" })
        ] })
      ]
    }
  ) });
...
function ConfirmRemoveAccount({ account, onCancel, onConfirm }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(Modal, { title: "Remove account", onClose: onCancel, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "form-stack", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "confirm-copy", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: "This removes the account from Bi-talks. This action cannot be undone." })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-actions", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "ghost-button", type: "button", disabled: removing, onClick: onCancel, children: "Cancel" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "danger-button", type: "button", disabled: removing, onClick: () => void confirm(), children: removing ? "Removing..." : "Remove" })
    ] })
  ] }) });
}
function ConfirmCloseApplication({ onCancel, onConfirm }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(Modal, { title: "关闭 Bi-Talks", onClose: onCancel, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "form-stack close-confirm-content", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "close-confirm-symbol", "aria-hidden": "true", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Power, { size: 24 }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "confirm-copy close-confirm-copy", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: "确定要关闭软件吗？" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: "关闭后，当前运行的账号窗口也会安全退出。" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-actions", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "ghost-button", type: "button", disabled: closing, onClick: onCancel, children: "取消" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "danger-button", type: "button", disabled: closing, onClick: () => void confirm(), children: closing ? "正在关闭..." : "确认关闭" })
    ] })
  ] }) });
}
function SettingsModal({ state, save, onClose, onNotice }) {
  ...
  const [groqApi, setGroqApi] = reactExports.useState(() => structuredClone(state.settings.groqApi));
  ...
  const persistSettings = async (overrides = {}) => {
    ...
    const normalizedGroqApi = {
      ...groqApi,
      apiKeys: groqApi.apiKeys.map((key) => ({
        ...key,
        name: key.name.trim(),
        projectId: key.projectId.trim() || "default",
      ...
      maxConcurrencyPerProject: Math.min(6, Math.max(1, Math.round(groqApi.maxConcurrencyPerProject || 2)))
    };
    const normalizedDeepLApiKey = deeplApiKey.trim();
    const nextApiUsage = {
      ...latestState.settings.apiUsage,
      groq: latestState.settings.apiUsage.groq,
      groqByKey: latestState.settings.apiUsage.groqByKey,
  ...
  const updateGroqApiKey = (id, field, value) => {
    setGroqApi((current) => ({
      ...current,
      apiKeys: current.apiKeys.map((key) => key.id === id ? { ...key, [field]: value } : key)
    }));
    setApiTestMessage("");
  };
  const addGroqApiKey = () => {
    setGroqApi((current) => ({
      ...
      apiKeys: [...current.apiKeys, {
        id: createGroqApiKeyId(),
        name: `Key ${current.apiKeys.length + 1}`,
        projectId: "default",
        apiKey: "",
        enabled: true
      }]
  ...
  const removeGroqApiKey = (id) => {
    setGroqApi((current) => current.apiKeys.length <= 1 ? current : { ...current, apiKeys: current.apiKeys.filter((key) => key.id !== id) });
    setApiTestMessage("");
  };
  const testApi = async () => {
    setTestingApi(true);
    ...
    try {
      if (translationProvider === "groq") {
        const keys = groqApi.apiKeys.filter((key) => key.enabled && key.apiKey.trim());
        if (!keys.length) throw new Error("请至少配置并启用一个 Groq API Key。");
        const results = [];
        for (const key of keys) {
          const result = await window.desktop.testTranslationApi("groq", key.apiKey.trim(), {
            ...groqApi,
            apiKeys: [key]
          });
          results.push(`${key.name || key.id}：${result.ok ? "成功" : result.message.replace(/^Groq API 验证失败：/, "")}`);
        }
        const failed = results.filter((message) => !message.endsWith("：成功"));
        setApiTestMessage(failed.length ? `部分 Key 验证失败：${results.join("；")}` : `全部 ${keys.length} 个 Groq Key 验证成功。`);
        if (!failed.length) await save(await window.desktop.getState());
      } else {
        const result = await window.desktop.testTranslationApi("deepl", deeplApiKey.trim());
  ...
  const resetCumulativeApiUsage = async () => {
    ...
    try {
      ...
      const resetGroqByKey = provider === "groq" ? Object.fromEntries(Object.entries(latestState.settings.apiUsage.groqByKey).map(([id, entry]) => [id, {
        ...entry,
        usage: {
          ...entry.usage,
          totalInputTokens: 0,
      ...
      }])) : latestState.settings.apiUsage.groqByKey;
      await save({
        ...latestState,
        settings: {
          ...latestState.settings,
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(Modal, { title: "Settings", onClose, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "form-stack", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "settings-section", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: "Appearance" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: "Choose the app theme. Glass effects stay on by default." })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "segmented-control", role: "radiogroup", "aria-label": "Theme mode", children: themeOptions.map((option) => /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          type: "button",
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
                ...
                min: 1,
                max: 6,
                value: groqApi.maxConcurrencyPerProject,
                onChange: (event) => updateGroqApi("maxConcurrencyPerProject", Number(event.currentTarget.value))
              }
            )
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "groq-key-list", children: groqApi.apiKeys.map((key) => {
          const keyUsage = state.settings.apiUsage.groqByKey[key.id]?.usage || emptyTranslationApiUsage();
          return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "groq-key-card", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "groq-key-card-header", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "switch-row", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "input",
                  {
                    type: "checkbox",
                    checked: key.enabled,
                    onChange: (event) => updateGroqApiKey(key.id, "enabled", event.currentTarget.checked)
                  }
                ),
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "switch-control", "aria-hidden": "true" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "启用" })
              ] }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "button",
                {
                  className: "icon-button groq-key-remove",
                  type: "button",
                  title: "删除此 Key",
                  disabled: groqApi.apiKeys.length <= 1,
                  onClick: () => removeGroqApiKey(key.id),
                  children: /* @__PURE__ */ jsxRuntimeExports.jsx(Trash2, { size: 15 })
                }
              )
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "groq-key-meta-grid", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { children: [
                "Key 名称",
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "input",
                  {
                    ...
                    onChange: (event) => updateGroqApiKey(key.id, "name", event.currentTarget.value)
                  }
                )
              ] }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { children: [
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "input",
                  {
                    value: key.projectId,
                    maxLength: 100,
                    placeholder: "同一 Project 请填写相同标识",
                    onChange: (event) => updateGroqApiKey(key.id, "projectId", event.currentTarget.value)
                  }
                )
              ] })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "input",
                {
                  ...
                  onChange: (event) => updateGroqApiKey(key.id, "apiKey", event.currentTarget.value)
                }
              )
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("small", { children: [
              "累计 ",
              numberFormat.format(keyUsage.totalTokens),
              " Token · 今日 ",
              numberFormat.format(keyUsage.dailyTokens),
              " Token"
            ] })
          ] }, key.id);
        }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "ghost-button groq-key-add", type: "button", onClick: addGroqApiKey, children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Plus, { size: 15 }),
          " 添加 Groq Key"
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: "同一 Project 的 Key 共享限额并作为主备；不同 Project 会轮换使用。遇到 429 会按官方等待时间退避。" })
      ] }),
      translationProvider === "deepl" && /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "input",
          {
            ...
            autoComplete: "off",
            spellCheck: false,
            placeholder: "粘贴 DeepL API Key（Free 密钥自动使用免费端点）",
            onChange: (event) => setDeeplApiKey(event.currentTarget.value)
          }
        )
      ] }),
      apiUsage && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "api-usage-card", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "api-usage-header", children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "api-usage-actions", children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "ghost-button api-test-button", type: "button", disabled: testingApi, onClick: () => void testApi(), children: testingApi ? "测试中…" : translationProvider === "groq" ? "测试全部 Key" : "测试 API" })
          ] })
        ] }),
        ...
        apiTestMessage && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: apiTestMessage.includes("失败") ? "api-test-error" : "api-test-success", children: apiTestMessage })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-actions", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "ghost-button", onClick: onClose, children: "Cancel" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "primary-button", onClick: () => void submit(), children: "Save" })
    ] })
  ] }) });
}
