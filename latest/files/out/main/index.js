...
`, true).catch(() => false);
  ...
  async recoverWhatsAppDatabase(account, senderId) {
    ...
    const now = Date.now();
    const recovery = this.whatsappDatabaseRecovery.get(account.id) || { stage: 0, lastReportAt: 0, inProgress: false };
    if (recovery.inProgress) return void 0;
    if (now - recovery.lastReportAt > 2 * 6e4) recovery.stage = 0;
    recovery.inProgress = true;
    recovery.lastReportAt = now;
    ...
    try {
      if (recovery.stage === 0) {
        ...
        return "storage-flushed-and-reloaded";
      }
      if (recovery.stage === 1) {
        recovery.stage = 2;
        try {
        ...
        if (!view.webContents.isDestroyed()) view.webContents.reloadIgnoringCache();
        return "cache-and-service-worker-rebuilt";
      }
      if (recovery.stage === 2) {
        recovery.stage = 3;
        this.disposeView(account.id, false, false);
        await reopenView();
        return "account-view-recreated";
      }
      if (recovery.stage === 3) {
        recovery.stage = 4;
        this.disposeView(account.id, false, false);
        ...
        await accountSession.flushStorageData();
        await reopenView();
        return "damaged-account-storage-rebuilt";
      }
      this.emit({
        type: "error",
...
`;
  ...
  async performBundledDesktopUpdate(account, localOnly = false) {
    ...
    try {
      ...
      this.emit({ type: "signal-update", accountId, status: "complete", message: `Signal 已更新到 ${targetVersion}`, version: targetVersion });
      return result;
    } catch (error) {
...
using System;
using System.Runtime.InteropServices;
public class Win32SignalShutdown {
  [DllImport("user32.dll")] public static extern bool ShowWindow(IntPtr hWnd, int nCmdShow);
}
"@
foreach ($hwnd in $hwnds) { [Win32SignalShutdown]::ShowWindow($hwnd, 0) | Out-Null }
`;
      runPowerShellSync(command);
    }
...
async function performSignalDesktopUpdateAndRestart(account) {
  ...
  const runningAccountIds = new Set(signal.runningAccountIds());
...
function recoverAfterForeground() {
  ...
  foregroundRecoveryTimer = setTimeout(() => {
    ...
    try {
      mainWindow.setBackgroundColor("#090c12");
